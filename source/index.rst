...
Application Processor and Supervisory Controller Processor Communications
=========================================================================
...
The Application Processor (AP) and the Supervisory Controller (SVC) need to communicate [ab]together in order to handle control over the different Greybus modules in the system. Though the exact nature of this channel is as-yet-unspecified, we make the following assumption that should hold true regardless of the exact implementation:

1. At system boot, the SVC discovers through some mechanism the UniPro network topology and the location of the AP.
2. It configures a UniPro route on the switch between the AP and the SVC on control CPort0.
3. Control CPort0 is a connection-oriented bidirectional stream of packets with datagram semantics over the L4 transport layer within the UniPro specification.

This section covers the software protocol [ac]between the SVC and the AP.

**TODO** insert missing figure

Supervisory Controller Service Requirements
-------------------------------------------

The Supervisory Controller (SVC) is a microcontroller embedded within the endoskeleton that manages core functionality of a Project Ara phone. Its functional requirements are:
1. Manage Unipro link and network states, establish routes[ad][ae] on the switch.
2. Handle system suspend and resume via wake/detect mechanisms and notifications of module hotplug/hot-unplug[af][ag]
3. Monitor module power and communicate battery state to the AP
4. Electro-permanent magnet management to physically lock and unlock modules from the endoskeleton
5. Perform module authentication, DDB retrieval/enumeration and validation of module descriptors and capabilities
Each of these services is implemented as a higher-level service protocol over a control CPort between the AP and the SVC.

Session Layer Service Establishment and Multiplexing
----------------------------------------------------

In order to begin communication, a session must be established between the AP and the SVC. This communication occurs over CPort0, the control CPort.

Due to interface restrictions and a limited number of CPorts, we have to multiplex each of the services above over a single CPort between the AP and the SVC. This can roughly be thought of as a “virtual CPort.” Toshiba calls these “Functions[ah].” Each side is responsible for establishing a session, multiplexing and demultiplexing the stream, packet reassembly and delivering it to each upper layer service appropriately.

Every packet over CPort0 is prepended by a session packet header[ai]. The session header consists of the function number, a packet type, and an optional payload of the service protocol that it implements:
...
.. list-table::
   ...
   * - Payload length
     - 2
     - Length of payload, in bytes, little-endian format.
...
Before any Functions are exposed, a session must be established by means of a handshake mechanism initiated by the SVC[aj][ak][al]. A handshake payload consists of a version identifier and a single byte identifying the source of the handshake:
...
.. list-table::
   ...
   * - Version Minor
     - 1
     - Minor version of this document
   * - Handshake Source
     - 1
     - Hello packet, (SVCHello =  0x0, APHello = 0x1, ModuleHello = 0x2)

UniPro Network Management Function
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

This service protocol implements UniPro network functionality. This includes commands sent from the AP to the SVC to request network management such as route setting, CPort bandwidth settings, as well as notifications from the SVC to the AP of Unipro network events such as link-up indications from newly discovered modules. Messages consist of either a command or a notification with the following structure:
...
**TODO** reinstate various of these AP/SVC protocols, if we actually want to keep them (they're commented out at the moment).

   ..
      AP_ID provides information the AP about its assigned module and device IDs. An AP_ID event shall have the following fields:
      ...
      Field
              ...
              Device ID
              LINK_UP
      LINK_UP is a message sent from the SVC to the AP whenever the SVC detects that a new module has enabled new UniPro link on the network. Whenever the SVC detects that a module has attached to the network, it will assign a UniPro device ID and notify the AP. A LINK_UP shall have the following fields:
      ...
      Field
              ...
              module_id
              1
              Module ID associated with the interface link
              interface_id
              1
              Interface ID of link
              device_id
              1
              Device ID assigned to interface


      SET_ROUTE
      SET_ROUTE requests a UniPro route between an source CPort and a destination CPort.
      A SET_ROUTE command shall have the following fields:
      ...
      Field
              ...
              Hotplug Function
      The hotplug function is a service that the SVC implements to notify the AP of modules that are physically plugged and unplugged. How the SVC discovers hotplugged and hot-unplugged modules is outside the scope of this document. However, when the SVC discovers a module, it must assign a unique Module ID for that module[am][an][ao]. The Module ID persists until the module is hot-unplugged and is used as the identifier for all module-level commands passing between the AP and SVC. A hotplug packet shall have the following structure:
      ...
      Field
              ...
              payload
              variable
              hotplug event payload


      The following hotplug event types are defined for this function:
      ...
      A HOTPLUG_EVENT has a payload that contains the full module manifest to describe the module that has just been added to the system. See the section above for the details on what the module manifest contains.


      A HOTUNPLUG_EVENT has no payload, any host data associated with a module that has previously been seen shall now be removed from the system.


      Power/Battery Management Function
      This function defines status requests and status notifications of power and battery related events. It has the following structure:
      ...
      Field
              ...
              module_id
              1
              Module ID of the battery module in question
              payload
              variable
              depends on power_type


      The following types are defined for this function:
      ...
      BATTERY_STATUS
      A battery status event is a message from the SVC to the AP that describes a battery’s current state. It can be generated either as a response to a BATTERY_STATUS_REQUEST from the AP, or asynchronously due to changes in the battery state. It has the following payload:
      Field
              Size
              Value
              Description
              ...
              Full charge value in mW
              charge_now
              2


              Charge Now value in mW
              status
              1
      ...
      Valid values for status are:
              ...
              4        full
      BATTERY_STATUS_REQUEST
      BATTERY_STATUS_REQUEST is a request from the AP to the SVC to provide a BATTERY_STATUS response of the battery module specified in module_id.  It has a zero-byte payload.
      EPM Management Function
      This function defines EPM management[as] commands from the AP to the SVC. It has the following structure:
      Field
              Size
              Description
              epm_command_type
      ...
      Suspend Management Function
      This function defines Suspend management commands from the AP to the SVC[at]. It has the following structure:
      Field
              Size
              Description
...
.. ----------------------------------------------------------------------
.. END: "OLD" (pre-operations) Greybus spec
.. ----------------------------------------------------------------------
