...
Greybus Human Interface Device (HID) protocol. The HID class consists primarily
of devices that take input from humans or may give output to humans.
...
The HID protocol deals with descriptors and reports for all interactive with the
HID device. It receives all capabilities of the HID device in a well defined
format, called as Descriptor. Based on these capabilities of the device, the
host may configure the device with help of feature-report before getting or
setting any data from/to the HID device with help of input/output-reports.

This document doesn't specify the internals of HID protocol itself, which is
well defined by `USB-HID <http://www.usb.org/developers/hidpage/HID1_11.pdf>`_
specifications, but defines how it is implemented with Greybus in place of USB.

Greybus HID Descriptors
^^^^^^^^^^^^^^^^^^^^^^^
The following section identifies the key data structures (referred to as
Descriptors) that need to be exchanged between the host and the device during
the startup phase.
...
based HID device must have. The purpose of the HID descriptor is to share all
capabilities of the HID device with the host. These attributes describe the
version that the protocol is compliant towards, length of descriptors, and other
capabilities of the device. Please refer to Table :num:`table-hid-descriptor`
for further details.

HID Report Descriptor
"""""""""""""""""""""
A Report descriptor describes the data generated by the HID device and how to
interpret that data. Details of the report descriptor are outside of the scope
of this document and are defined in `USB-HID
<http://www.usb.org/developers/hidpage/HID1_11.pdf>`_ specifications.

HID Report Protocol
^^^^^^^^^^^^^^^^^^^
The Report is the fundamental block exchanged between the host and the device.
Reports are well defined by the `USB-HID
<http://www.usb.org/developers/hidpage/HID1_11.pdf>`_ Class Specification and
...
.. c:function:: int get_report_descriptor(u8 *report_desc);
    ...
    Returns report descriptor, defined by `USB-HID
    <http://www.usb.org/developers/hidpage/HID1_11.pdf>`_ specification.
...
The Greybus HID get descriptor operation request is sent from host to device and
it has no payload.
...
.. figtable::
    ...
    3        bcdHID             2       Number          Version of the HID protocol, as defined by `USB-HID <http://www.usb.org/developers/hidpage/HID1_11.pdf>`_
...
device must respond with an report descriptor as defined by `USB-HID
<http://www.usb.org/developers/hidpage/HID1_11.pdf>`_ specification.

Greybus HID Get Report Descriptor Request
"""""""""""""""""""""""""""""""""""""""""

The Greybus HID get report descriptor operation is sent from host to device and
...
it consists of a report descriptor defined by `USB-HID
<http://www.usb.org/developers/hidpage/HID1_11.pdf>`_ specification.
...
The Greybus HID Get Report response returns report as defined by `USB-HID
<http://www.usb.org/developers/hidpage/HID1_11.pdf>`_ specification.
...
The Greybus HID set report request contain report-type, report-id and report (as
defined by `USB-HID <http://www.usb.org/developers/hidpage/HID1_11.pdf>`_
specification) as defined in Table :num:`table-hid-set-report-request`.
...
.. figtable::
    ...
    =======  ==============  ======  ==========      ===========================
    0        Report Type     1       Number          :ref:`hid-report-type`
    1        Report ID       1       Number          Report ID defined by `USB-HID <http://www.usb.org/developers/hidpage/HID1_11.pdf>`_ specifications
    2        Report          ...     Data            Report defined by `USB-HID <http://www.usb.org/developers/hidpage/HID1_11.pdf>`_ specification
    =======  ==============  ======  ==========      ===========================
...
The format of the input-report is defined by `USB-HID
<http://www.usb.org/developers/hidpage/HID1_11.pdf>`_ specification.
