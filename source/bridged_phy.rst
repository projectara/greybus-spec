...
The I2S protocol is designed to allow an I2S transmitter to stream data
directly to an I2S receiver.  In addition, an I2S transmitter may stream
data to more than one receiver module.  To support this, the controlling
module (AP) must set up control connections between it and all of the I2S
transmitter and receiver modules that will be involved in the I2S data
transfer.  The control connections will be standard UniPro connections
with full E2EFC enabled.

The data connections will directly connect I2S transmitters to I2S receivers.
Data connections have E2EFC disabled with CSD and CSV enabled.  The control
and data connections support different protocols.  Both protocols are
described in this section



**XXX PUT DIAGRAM HERE XXX**



The above is an illustration showing the various I2S data, control as well
as other Greybus protocol connections that would be active in one possible
audio routing configuration (AP telephone ring over a loudspeaker, and
modem to ear speaker/mic routing for the voice call).

The Greybus message sequence required to carry out a phone conversation
based on the above illustration could go like this:

*   AP module sets up all of the I2S Control and Data connections shown
    in the illustration (see Control Protocol section).
*   AP module sends Get Version Requests messages to all modules involved
    and ensures that it communicates with those modules using a supported
    Greybus protocol version.
*   AP module sends an Add Receiver Request message to the Modem module
    requesting that it add the specified local CPort that represents the
    I2S connection from the Modem module to the Ear Speaker module.
*   AP module sends an Add Receiver Request message to the Mic module
    requesting that it add the specified local CPort that represents the
    I2S connection from the Mic module to the Modem module.
*   AP module send an Add Receiver Request message to the AP modules
    itself with the specified local CPort that represents the I2S
    connection from the AP module to the Loud Speaker module.
*   Since there is no video stream to synchronize the audio with,
    the AP module does not need the information provided by the
    Get Processing Delay Request message.  If there was a video stream
    associated with this audio connection, it would use the processing
    delay information to calculate how long to set the start delay for
    each module and send those modules Set Delay Request messages.
*   AP module send Get Supported Configurations Request messages to all
    of the modules, determines the best common configuration, and sends
    Set Configuration Request messages back to the modules with that
    configuration.  All modules are now configured similarly.
*   AP module sends Prepare Transmitter Request messages to the Modem
    module, Mic module, and the AP module itself so they power up and
    initialize their I2S transmitting subsystems.
*   AP module sends a Prepare Receiver Request messages to the Ear
    Speaker module, Modem module, and Loud Speaker module so they
    power up and initialize their I2S receiving subsystems.
*   AP module sends Start Transmitter Request messages to all of the
    modules that received Prepare Transmitter Request messages.
*   Each transmitting module issues Send Data Request messages to
    each local CPort that is on its list of receiving CPorts (set
    up by Add Receiver Request messages).  The audio streams are now
    fully active.
*   If any of the transmitting or receiving modules encounters
    significant problems, they will send Report Event Request
    messages to the AP module.
*   AP module sends Stop Transmitter Request messages to all of the
    modules that received Start Transmitter Request messages, once
    the call is complete.
*   AP module sends Shutdown Transmitter Request messages to all of
    the modules that received Stop Transmitter Request messages.
    The transmitter can now power down its I2S subsystem.
*   AP module sends Shutdown Receiver Request messages to all of
    the modules that received Prepare Receiver Request message.
    The receiver can now power down its I2S subsystem.
*   AP module sends a matching Remove Receiver Request message for
    each Add Receiver Request message it sent earlier.

Conceptually, the I2S control protocol operations made between the AP and
the transmitting and receiving modules are:

.. c:function:: int get_version(u8 *major, u8 *minor);

    Returns the major and minor Greybus I2S protocol version number
    supported by the I2S controller. I2S controllers adhering to the
    protocol specified herein shall report major version 0, minor version 1.

.. c:function:: int add_receiver(u8 cport);

    Requests the I2S transmitter add a new I2S receiver module to the
    list of receivers that it streams I2S data to.  ‘cport’ is the cport
    local to the transmitter which is already connected to an I2S receiver.

.. c:function:: int remove_receiver(u8 cport);

    Requests the I2S transmitter remove a receiver from the list of
    receivers that it streams I2S data to. The ‘cport’ is the local
    cport on the transmitter which is connected to an I2S receiver.

.. c:function:: int get_processing_delay(u32 *microseconds);

    Returns the number of microseconds the module requires to process
    its data before it is sent on local CPort or passed to the output
    device.

.. c:function:: int set_start_delay(u32 delay);

    Requests the I2S controller delay for the specified number of
    microseconds before it starts sending data.  This message may
    be sent to either a transmitter or a receiver.  When sent to
    a transmitter, the transmitter will delay the specified number
    of microseconds before its starts sending messages to the receiver.
    When sent to a receiver, the receiver will delay the specified
    number of microseconds before it starts sending the I2S data on
    its local I2S connection.

.. c:function:: int get_supported_configurations(u16 *configurations, struct gb_i2s_configuration *configurations);

    Requests the I2S controller return all of its supported configurations.

.. c:function:: int set_configuration(struct gb_i2s_configuration *configuration);

    Requests the I2S controller set its configuration’s values to the
    one specified.  All transmitters and receivers shall be set to
    the configuration.

.. c:function:: int set_samples_per_message(u16 samples_per_message);

    Requests the I2S controller send the specified number of I2S
    samples in each Greybus message (i.e., in each send_data() request).

.. c:function:: int prepare_receiver(u8 cport);

    Requests the I2S receiver power up and initialize so that its
    ready to start receiving.

.. c:function:: int shutdown_receiver(void);

    Notifies the receiver that it can release its resources and power down.

.. c:function:: int prepare_transmiter(void);

    Requests the I2S transmitter power up and initialize so that its
    ready to start transmitting.

.. c:function:: int start_transmiter(void);

    Requests the I2S transmitter start sending data to its list of
    I2S receivers.  There shall be at least one receiver added to
    the transmitter’s receiver list and all of the receivers on the
    list shall be successfully prepared before sending this message.

.. c:function:: int stop_transmiter(void);

    Requests the I2S transmitter stop sending data to its list of
    I2S receivers.
.. c:function:: int shutdown_transmiter(void);

    Notifies the transmitter that it can release its resources
    and power down.

.. c:function:: int mute(void);

    Requests the I2S controller to mute.

.. c:function:: int unmute(void);

    Requests the I2S controller to unmute.

.. c:function:: int report_event(u32 event);

    Reports an event from transmitter or receiver to the AP.

Conceptually, the I2S data protocol operations made directly between
transmitting and receiving modules are:

.. c:function:: int send_data(u32 sample_number, u32 size, u8 *data);

    Requests the I2S transmitter send the I2S stream data.
    All data will be in binary twos compliment.  The number
    of bytes of data for each channel shall be equal and a
    power of 2.  Sample number is the number of the first
    sample in the message.  Sample numbers start at zero
    and count up by one for each sample.

The format field in the configuration structure specifies the low-level
protocol used to stream data over the 3- (or more) wire interface local
to the ARA module.  This level of detail is unnecessary for streaming
over the UniPro network and doesn’t always change the format of the
Send Data Request messages.  The reason for the detail is to give the
controlling AP module (e.g., the Application Processor module (AP)) the
information it requires to configure the codec/mixer/etc. that is at
the far end of the physical connection.  Otherwise, control of the
codec/mixer/etc. would be shared between the AP and the local GPBridge
making that control more complex to implement.

The byte order field in the configuration structure could be set by
issuing separate Greybus messages but is currently included in the
structure so that complete configuration can be accomplished with
one operation.

Both of these may change in future versions of this protocol.

Greybus I2S Message Types
^^^^^^^^^^^^^^^^^^^^^^^^^

This table describes the Greybus I2S operation types and their
values.  A message type consists of an operation type combined with a
flag (0x80) indicating whether the operation is a request or a
response.  All operations have responses except for Send Data
Request.

============================  =============  ==============
I2S Control Operation Type    Request Value  Response Value
============================  =============  ==============
Invalid                       0x00           0x80
Protocol Version              0x01           0x81
Add receiver                  0x02           0x82
Remove receiver               0x03           0x83
Get processing delay          0x04           0x84
Set start delay               0x05           0x85
Get supported configurations  0x06           0x86
Set configuration             0x07           0x87
Set samples per message       0x08           0x88
Prepare receiver              0x09           0x89
Shutdown receiver             0x0a           0x8a
Prepare transmitter           0x0b           0x8b
Start transmitter             0x0c           0x8c
Stop transmitter              0x0d           0x8d
Shutdown transmitter          0x0e           0x8e
Mute                          0x0f           0x8f
Unmute                        0x10           0x90
Report event                  0x11           0x91
...
Greybus I2S Protocol Version Operation
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The Greybus I2S protocol version operation allows the AP to determine
the version of this protocol to which the I2S adapter complies.

Greybus I2S Protocol Version Request
""""""""""""""""""""""""""""""""""""

The Greybus I2S protocol version request contains no data beyond the
Greybus I2S message header.

Greybus I2S Protocol Version Response
"""""""""""""""""""""""""""""""""""""

The Greybus I2S protcol version response contains a status byte,
followed by two 1-byte values. If the value of the status byte is
non-zero, any other bytes in the response shall be ignored. A Greybus
I2S adapter adhering to the protocol specified herein shall report
major version 0, minor version 1.
...
=======  ==============  ======  ==========      ===========================
0        status          1       Number          Success, or reason for failure
1        version_major   1       |gb-major|      I2S protocol major version
2        version_minor   1       |gb-minor|      I2S protocol minor version
=======  ==============  ======  ==========      ===========================

Greybus I2S Add Receiver Operation
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The Greybus I2S Add Receiver operation requests the I2S transmitter add a
new receiver to its list of I2S data receivers.  It shall stream I2S data
to the new receiver along with the other receivers that it is already
streaming I2S data to.

Greybus I2S Add Receiver Request
""""""""""""""""""""""""""""""""

The Greybus I2S Add Receiver request supplies the cport that the I2S
transmitter will start streaming data to.  The cport address is local
to the I2S transmitter.

=======  ==============  ======  ==========      ===========================
Offset   Field           Size    Value           Description
=======  ==============  ======  ==========      ===========================
0        cport           1       Number          Local cport to start
                                                 streaming I2S data to
=======  ==============  ======  ==========      ===========================

Greybus I2S Add Receiver Response
"""""""""""""""""""""""""""""""""

The Greybus I2S Add Receiver response contains only the status byte.
...
Greybus I2S Remove Receiver Operation
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The Greybus I2S Remove Receiver operation requests the I2S transmitter
remove a receiver from its list of I2S data receivers.  It shall no
longer stream I2S data to the specified receiver.

Greybus I2S Remove Receiver Request
"""""""""""""""""""""""""""""""""""

The Greybus I2S Remove Receiver request supplies the cport that the
I2S transmitter will stop streaming data to.  The cport address is
local to the I2S transmitter.

=======  ==============  ======  ==========      ===========================
Offset   Field           Size    Value           Description
=======  ==============  ======  ==========      ===========================
0        cport           1       Number          Local cport to stop
                                                 streaming I2S data to
=======  ==============  ======  ==========      ===========================

Greybus I2S Remove Receiver Response
""""""""""""""""""""""""""""""""""""

The Greybus I2S Remove Receiver response contains only the status byte.
...
I2S controller indicate what its data processing delay is once
its running.  Delay values are in microseconds.
...
=======  ==============  ======  ==========      ===========================
0        status          1       Number          Success, or reason for failure
1        delay           4       Number          Processing delay
                                                 (microseconds)
=======  ==============  ======  ==========      ===========================
...
controller delay the start of processing the specified amount
of time.  Delay values are in microseconds.

Greybus I2S Set Start Delay Request
"""""""""""""""""""""""""""""""""""

The Greybus I2S Set Start Delay request supplies the delay that
the controller shall wait before starting its processing.

=======  ==============  ======  ==========      ===========================
Offset   Field           Size    Value           Description
=======  ==============  ======  ==========      ===========================
0        delay           4       Number          Delay before starting
                                                 (microseconds)
=======  ==============  ======  ==========      ===========================
...
the I2S controller return an array containing all of the configurations
that it supports.  Each configuration contains 4-byte sample frequency,
a 4-bytes format, a 1-byte number indicating the number of bytes per
channel on each sample, and a 1-byte value indicating the byte ordering
when there are 2 or more bytes per channel.  The number of bytes per
channel shall be greater than 0 and a power of 2.
...
the status byte, followed by a 2-byte value specifying the
configuration count, and an array of configuration descriptors.

**Greybus I2S Config**

A Greybus I2S Configuration describes a supported I2S configuration.

=======  =================  ======  ==========      ===========================
Offset   Field              Size    Value           Description
=======  =================  ======  ==========      ===========================
0        sample_frequency   4       Number          Samples per second
4        format             4       Number          :ref:`i2s-format-bits`
8        bytes_per_channel  1       Number          Number of bytes per channel
                                                    in each sample
9        byte_orders        1       Number          :ref:`i2s-byte-order-bits`
=======  =================  ======  ==========      ===========================

.. _i2s-format-bits:

Greybus I2S Format Bits
"""""""""""""""""""""""

This table describes the format bit values for Greybus I2S modules.

===============================  ========================       ===============
Symbol                           Brief Description              Mask Value
===============================  ========================       ===============
GB_I2S_FORMAT_PCM_MONO           Single channel PCM (sync       0x01
                                 when LRCLK low)
GB_I2S_FORMAT_PCM_MONO_REV_POL   Single channel PCM (sync       0x02
                                 when LRCLK high)
GB_I2S_FORMAT_I2S_STEREO         True I2S stereo                0x04
GB_I2S_FORMAT_LR_STEREO          LR stereo (left channel        0x08
                                 when LRCLK low)
GB_I2S_FORMAT_LR_STEREO_REV_POL  LR stereo (left channel        0x10
                                 when LRCLK high)
===============================  ========================       ===============
...
This table describes the byte-order bit values for Greybus I2S modules.

===============================  ========================       ===============
Symbol                           Brief Description              Mask Value
===============================  ========================       ===============
GB_I2S_BYTE_OREDER_BE            Big endian                     0x01
GB_I2S_BYTE_OREDER_LE            Little endian                  0x02
===============================  ========================       ===============

Here is the structure of a Greybus I2S get configuration response.

===========  ==============  ======  =================  =======================
Offset       Field           Size    Value              Description
===========  ==============  ======  =================  =======================
0            status          1       Number             Success, or reason for
                                                        failure
1            config_count    4       Number             Number of entries in
                                                        configuration array
5            config[1]       10      struct i2s_config  Descriptor for first
                                                        I2S configuration
...          ...             10      struct i2s_config  ...
5+10*(N-1)   config[N]       10      struct i2s_config  Descriptor for Nth I2S
                                                        configuration
===========  ==============  ======  =================  =======================
...
The Greybus I2S Set Configuration operation requests the I2S controller
...
that the controller shall configure itself to use.

=======  ==============  ======  =================  ===========================
Offset   Field           Size    Value              Description
=======  ==============  ======  =================  ===========================
0        config           10     struct i2s_config  I2S configuration
=======  ==============  ======  =================  ===========================

Greybus I2S Set Congiruation Response
"""""""""""""""""""""""""""""""""""""
...
I2S transmitter set the number samples it sends in each Greybus
Send Data message to the specified value.

Greybus I2S Set Samples per Message Request
"""""""""""""""""""""""""""""""""""""""""""

The Greybus I2S Set Samples per Message request supplies the number
of samples that the transmitter shall put in each Greybus I2S Send
Data message.
...
Greybus I2S Prepare Receiver Operation
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The Greybus I2S Prepare Receiver operation requests the
I2S receiver controller prepare itself to begin receiving
Greybus I2S Send Data request messages on the specified cport.

Greybus I2S Prepare Receiver Request
""""""""""""""""""""""""""""""""""""

The Greybus I2S Prepare Receiver request supplies the cport
that I2S streaming data will arrive on.  The cport address
is local to the I2S receiver.

=======  ==============  ======  ==========      ===========================
Offset   Field           Size    Value           Description
=======  ==============  ======  ==========      ===========================
0        cport           1       Number          Local cport to receive
                                                 streaming I2S data on
=======  ==============  ======  ==========      ===========================

Greybus I2S Prepare Receiver Response
"""""""""""""""""""""""""""""""""""""

The Greybus I2S Prepare Receiver response contains only the status byte.
...
Greybus I2S Shutdown Receiver Operation
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The Greybus I2S Shutdown Receiver operation indicates to the
I2S receiver controller that it will no longer receive Greybus
I2S Send Data request messages and that it can free all resources
allocated by the Greybug Prepare Receiver operation.

Greybus I2S Shutdown Recaiever Request
""""""""""""""""""""""""""""""""""""""

The Greybus I2S Shutdown Receiver request contains no data
beyond the Greybus I2S message header.

Greybus I2S Shutdown Receiver Response
""""""""""""""""""""""""""""""""""""""

The Greybus I2S Shutdown Receiver response contains only the status byte.
...
Greybus I2S Prepare Transmitter Operation
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The Greybus I2S Prepare Transmitter operation requests the
I2S transmitter controller prepare itself to send Greybus
I2S Send Data request messages to its list of receiver controllers.

Greybus I2S Prepare Transmitter Request
"""""""""""""""""""""""""""""""""""""""

The Greybus I2S Prepare Transmitter request contains no data
beyond the Greybus I2S message header.

Greybus I2S Prepare Transmitter Response
""""""""""""""""""""""""""""""""""""""""

The Greybus I2S Prepare Transmitter response contains only the status byte.
...
Greybus I2S Start Transmitter Operation
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The Greybus I2S Start Transmitter operation requests the
I2S transmitter controller start sending Greybus I2S Send
Data request messages to its list of receiver controllers.

Greybus I2S Start Transmitter Request
"""""""""""""""""""""""""""""""""""""

The Greybus I2S Start Transmitter request contains no data
beyond the Greybus I2S message header.

Greybus I2S Start Transmitter Response
""""""""""""""""""""""""""""""""""""""

The Greybus I2S Start Transmitter response contains only the status byte.
...
Greybus I2S Stop Transmitter Operation
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The Greybus I2S Stop Transmitter operation requests that the
I2S transmitter controller stop sending Greybus I2S Send Data
request messages.

Greybus I2S Stop Transmitter Request
""""""""""""""""""""""""""""""""""""

The Greybus I2S Stop Transmitter request contains no data
beyond the Greybus I2S message header.

Greybus I2S Stop Transmitter Response
"""""""""""""""""""""""""""""""""""""

The Greybus I2S Stop Transmitter response contains only the status byte.
...
Greybus I2S Shutdown Transmitter Operation
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The Greybus I2S Shutdown Transmitter operation indicates
to the I2S transmitter controller that it shall no longer
send Greybus I2S Send Data request messages and that it
can free all resources allocated by the Greybug Prepare
Transmitter operation.

Greybus I2S Shutdown Transmitter Request
""""""""""""""""""""""""""""""""""""""""

The Greybus I2S Shutdown Transmitter request contains
no data beyond the Greybus I2S message header.

Greybus I2S Shutdown Transmitter Response
"""""""""""""""""""""""""""""""""""""""""

The Greybus I2S Shutdown Transmitter response contains only the status byte.
...
Greybus I2S Mute Operation
^^^^^^^^^^^^^^^^^^^^^^^^^^

The Greybus I2S Mute operation requests the I2S controller
mute its I2S stream.  Both transmitter and receiver controllers
shall implement this operation.

Greybus I2S Mute Request
""""""""""""""""""""""""

The Greybus I2S Mute request contains no data beyond the Greybus
I2S message header.

Greybus I2S Mute Response
"""""""""""""""""""""""""

The Greybus I2S Mute response contains only the status byte.
...
Greybus I2S Unmute Operation
^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The Greybus I2S Unmute operation requests the I2S controller
unmute its I2S stream.  Both transmitter and receiver controllers
shall implement this operation.

Greybus I2S Unmute Request
""""""""""""""""""""""""""

The Greybus I2S Unmute request contains no data beyond the
Greybus I2S message header.

Greybus I2S Unmute Response
"""""""""""""""""""""""""""

The Greybus I2S Unmute response contains only the status byte.
...
The Greybus I2S Report Event operation provides notification
to the AP module of events that are occurring on the
transmitter and receiver controllers while processing the audio
data stream.

Greybus I2S Report Event Request
""""""""""""""""""""""""""""""""

The Greybus I2S Report Event request supplies the 1-byte event
that has occurred on the sending controller.

=======  ==============  ======  ==========      ===========================
Offset   Field           Size    Value           Description
=======  ==============  ======  ==========      ===========================
0        event           1       Number          :ref:`i2s-event`
=======  ==============  ======  ==========      ===========================
...
This table describes the event values for Greybus I2S modules.

===============================  ========================       ===============
Symbol                           Brief Description              Value
===============================  ========================       ===============
GB_I2S_EVENT_PROTOCOL_ERROR      Incorrect operation            0x01
                                 order, etc.
GB_I2S_EVENT_OUT_OF_SEQUENCE     Sample sequence number           0x02
                                 lower than one already
                                 received
GB_I2S_EVENT_UNDERRUN            No data to send                0x03
GB_I2S_EVENT_OVERRRUN            Being flooded by data          0x04
GB_I2S_EVENT_CLOCKING            Low-level clocking issue       0x05
GB_I2S_EVENT_DATA_LEN            Invalid message data           0x06
                                 length
GB_I2S_EVENT_FAILURE             Operation failed               0x07
GB_I2S_EVENT_INTERNAL_ERROR      Internal error that            0x08
                                 should never happen
GB_I2S_EVENT_UNSPECIFIED         Catch-all for events           0x09
...
The Greybus I2S send data request supplies one or more complete
I2S samples in the currently selected I2S data format.
...
4        data_size       4       Number          Number of bytes in data
                                                 field
8        data            ...     Data            'N' complete samples of
                                                 audio data
=======  ==============  ======  ==========      ===========================
